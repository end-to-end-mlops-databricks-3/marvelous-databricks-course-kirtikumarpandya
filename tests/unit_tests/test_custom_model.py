...
def test_load_data_validate_df_assignment(mock_custom_model: BasicModel) -> None:
    """Validate correct assignment of train and test DataFrames from CSV files.

    :param mock_custom_model: Mocked BasicModel instance for testing.
    """
...
def test_load_data_validate_splits(mock_custom_model: BasicModel) -> None:
...
def test_prepare_features(mock_custom_model: BasicModel) -> None:
...
def test_train(mock_custom_model: BasicModel) -> None:
    ...
    mock_custom_model.load_data()
    mock_custom_model.prepare_features()
    mock_custom_model.finetune_parameters()
    expected_feature_names = (
        mock_custom_model.config.num_features
        + mock_custom_model.config.cat_features
        + mock_custom_model.config.date_features
        + mock_custom_model.config.id_cols
    )

    assert len(expected_feature_names) == 18  # date,  timestamp


def test_log_model_with_PandasDataset(mock_custom_model: BasicModel) -> None:
    ...
    assert len(expected_feature_names) == 18
...
def test_register_model(mock_custom_model: BasicModel) -> None:
    ...
    :param mock_custom_model: A mocked instance of the BasicModel class.
    """
    mock_custom_model.load_data()
    mock_custom_model.prepare_features()
    mock_custom_model.finetune_parameters()
    ...
    client = MlflowClient()
    model_name = f"{mock_custom_model.catalog_name}.{mock_custom_model.schema_name}.hotel_reservations_model"
...
def test_retrieve_current_run_metadata(mock_custom_model: BasicModel) -> None:
...
def test_load_latest_model_and_predict(mock_custom_model: BasicModel) -> None:
