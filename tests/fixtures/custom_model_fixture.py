...
def tags() -> Tags:
    ...
    """
    return Tags(git_sha="wxyz", branch="test")
...
def mock_custom_model(config: ProjectConfig, tags: Tags, spark_session: SparkSession) -> BasicModel:
    ...
    # Mock mlflow.data.from_spark - create a mock dataset object
    mock_dataset = MagicMock()

    # Create a mock for mlflow.data.from_spark function
    import mlflow
    import mlflow.data

    original_from_spark = mlflow.data.from_spark
    original_log_input = mlflow.log_input

    mlflow.data.from_spark = MagicMock(return_value=mock_dataset)
    mlflow.log_input = MagicMock()  # Mock the log_input function

    # Store original functions to restore later if needed
    instance._original_mlflow_from_spark = original_from_spark
    instance._original_mlflow_log_input = original_log_input
